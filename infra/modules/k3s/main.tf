...
curl -sfL https://get.k3s.io | sh -s - --write-kubeconfig-mode 644 --bind-address=0.0.0.0 --advertise-address=$PUBLIC_IP --node-external-ip=$PUBLIC_IP --tls-san=$PUBLIC_IP --tls-san=$PRIVATE_IP
