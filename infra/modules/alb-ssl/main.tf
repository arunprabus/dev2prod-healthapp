# Import self-signed certificate to avoid DNS validation
resource "tls_private_key" "k3s_key" {
  algorithm = "RSA"
  rsa_bits  = 2048
}

resource "tls_self_signed_cert" "k3s_cert" {
  private_key_pem = tls_private_key.k3s_key.private_key_pem

  subject {
    common_name = "${var.environment}.k3s.${var.domain_name}"
  }
...
resource "aws_acm_certificate" "k3s_cert" {
  ...
  certificate_body = tls_self_signed_cert.k3s_cert.cert_pem
